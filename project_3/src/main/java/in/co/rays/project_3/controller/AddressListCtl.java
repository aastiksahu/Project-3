...
@WebServlet(name = "AddressListCtl", urlPatterns = { "/ctl/AddressListCtl" })
public class AddressListCtl extends BaseCtl{

	private static Logger log = Logger.getLogger(AddressListCtl.class);
...
//		dto.setPinCode(DataUtility.getInt(request.getParameter("pinCode")));
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		log.debug("AddressListCtl doGet Start");
		...
		log.debug("AddressListCtl doGet End");
	}
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					AddressDTO deletedto = new AddressDTO();
					for (String id : ids) {
			...
			dto = (AddressDTO) populateDTO(request);
	...
	protected String getView() {
		...
		return ORSView.ADDRESS_LIST_VIEW;
	}
